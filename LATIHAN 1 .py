...
def check_password():
    if "password_correct" not in st.session_state:
        # --- CSS FIX: LEBARKAN KOTAK & FIX TEXT WRAP ---
        st.markdown("""
            <style>
            .stApp {
            ...
            /* FIX: Tambah min-width supaya teks "ACCESS ID" tak pecah dua */
            ...
            .glass-header { color: #d4a373; text-transform: uppercase; letter-spacing: 5px; font-size: 24px; font-weight: 700; margin-bottom: 2px; }
            .glass-sub { color: rgba(255, 255, 255, 0.7); font-size: 9px; letter-spacing: 4px; text-transform: uppercase; margin-bottom: 20px; }
            </style>
        ...
        with col_mid:
            ...
            st.markdown('<div style="display: flex; justify-content: center; margin-bottom: 15px;"><div style="background: #d4a373; padding: 10px; border-radius: 50%; width: 60px; height: 60px; display: flex; align-items: center; justify-content: center; box-shadow: 0 0 20px rgba(212, 163, 115, 0.5);"><img src="https://cdn-icons-png.flaticon.com/512/295/295128.png" width="35"></div></div>', unsafe_allow_html=True)
            ...
            if st.button("UNLOCK SYSTEM", use_container_width=True):
                if user_id == "zed" and password == "admin123":
                    ...
                    st.rerun()
                else: st.error("❌ Invalid Access ID or Password.")
...
if check_password():
    ...
    st.sidebar.markdown("""<div style="background: linear-gradient(135deg, #d4a373, #bc8a5f); padding: 20px; border-radius: 15px; text-align: center; margin-bottom: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.1);"><img src="https://cdn-icons-png.flaticon.com/512/3135/3135715.png" width="80" style="border-radius: 50%; border: 3px solid white;"><h3 style="color: white; margin-top: 10px; font-family: sans-serif;">Hai, Zed!</h3><p style="color: #f0f0f0; font-size: 0.8em; margin-bottom: 0px;">Surveyor Berdaftar</p></div>""", unsafe_allow_html=True)
    col_logo, col_text = st.columns([1.2, 4])
    with col_logo:
        if os.path.exists("Poli_Logo.png"): st.image("Poli_Logo.png", width=180)
        else: st.warning("⚠️ Logo 'Poli_Logo.png' tidak dijumpai.")
    with col_text:
        st.markdown("""<style>.main-title { font-family: 'Arial Black', Gadget, sans-serif; font-size: 55px; font-weight: 900; margin-bottom: -15px; line-height: 1; letter-spacing: -2px; color: #1e1e1e; } .sub-title { font-size: 20px; color: #d4a373; margin-top: 0px; font-weight: bold; }</style><div><h1 class="main-title">SISTEM SURVEY LOT</h1><p class="sub-title">Politeknik Ungku Omar | Jabatan Kejuruteraan Awam</p></div>""", unsafe_allow_html=True)
    ...
    poly_color = st.sidebar.color_picker("Warna Kawasan (Poligon)", "#d4a373") 
    line_color = st.sidebar.color_picker("Warna Garisan Sempadan", "#1e1e1e") 
    ...
    if uploaded_file is not None:
        try:
            ...
            if all(col in df.columns for col in ['STN', 'E', 'N']):
                ...
                geojson_dict = {"type": "FeatureCollection", "features": [{"type": "Feature", "geometry": mapping(poly_ll), "properties": {"Area_sqm": round(area, 2), "Stations": len(df)}}]}
                ...
                if show_interactive_map:
                    google_map_url = 'https://mt1.google.com/vt/lyrs=y&x={x}&y={y}&z={z}' if map_provider != "Standard Map" else 'https://mt1.google.com/vt/lyrs=m&x={x}&y={y}&z={z}'
                    ...
                    for i in range(len(df)):
                        ...
                        folium.Marker([ (p1['lat'] + p2['lat']) / 2, (p1['lon'] + p2['lon']) / 2], icon=folium.DivIcon(html=f'''<div style="transform: rotate({angle}deg); text-align: center; width: 160px; margin-left: -80px; margin-top: {v_offset}px;"><div style="font-size: {label_size_data}pt; color: white; text-shadow: 2px 2px 3px black; font-weight: bold;">{format_dms(bear)}<br><span style="color: #FFD700;">{dist:.2f}m</span></div></div>''')).add_to(m)
                    ...
                    bg_color, grid_color = ("#121212", "#555555") if plot_theme == "Dark Mode" else ("#003366", "#004080") if plot_theme == "Blueprint" else ("#ffffff", "#aaaaaa")
                    ...
                    if show_bg_grid:
                        ...
                        ax.xaxis.set_major_locator(plt.MultipleLocator(grid_interval)); ax.yaxis.set_major_locator(plt.MultipleLocator(grid_interval))
                    else: ax.axis('off')
                    if show_luas_label: ax.text(centroid_m.x, centroid_m.y, f"{area:.2f} m²", fontsize=label_size_luas, fontweight='bold', color='darkgreen', ha='center', bbox=dict(boxstyle='round,pad=0.3', fc='white', alpha=0.9, ec='green'), zorder=10)
                    for i in range(len(df)):
                        p1, p2 = df.iloc[i], df.iloc[(i + 1) % len(df)]
                        dE, dN = p2['E'] - p1['E'], p2['N'] - p1['N']
                        dist, bear = np.sqrt(dE**2 + dN**2), (np.degrees(np.arctan2(dE, dN)) + 360) % 360
